...
import emojiImg from "../../../assets/createTweet/emoji.svg";
    ...
    <div className="flex flex-col justify-between border-2 border-neutral-700 h-screen relative max-w-[550px]">
      <div>
        ...
        <div className="m-4 flex flex-col items-center border-b-2 border-neutral-700">
          ...
          <p className="my-4">Break the cycle, Rise Above</p>
        </div>

        <div className="mx-4 flex flex-col gap-3">
          <ReceivedMessage
            content={"Hey Shakti 👋, how are you"}
            time={"10:30 AM"}
          />
          <SentMessage content={"Hello Sandeep!"} time={"10:46 AM"} />
          <SentMessage
            content={"Im good, how are you though we havent met in ages"}
            time={"10:47 AM"}
          />
          <SentMessage
            content={"When are you coming to mumbai"}
            time={"10:47 AM"}
          />
          <ReceivedMessage content={"Guess what"} time={"10:49 AM"} />
          <ReceivedMessage
            content={
              "Im in mumbai only, I reached yesterday lets catch up where youre free"
            }
            time={"10:49 AM"}
          />
          <SentMessage
            content={"Sounds cool, I will call you"}
            time={"10:53 AM"}
          />
      ...
      <div className=" bg-neutral-700 flex mx-2 rounded-2xl mb-4">
        ...
        <button className="pr-2" onClick={() => setShowEmoji(!showEmoji)}>
          <Image src={emojiImg} alt="emoji" className="h-6 w-6" />
        </button>
        <button className="pr-2" onClick={handleSend}>
          <Image src={send} alt="send" className="h-6 w-6" />
        </button>
      </div>
    </div>
  );
